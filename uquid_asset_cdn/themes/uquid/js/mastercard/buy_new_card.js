(function ($) {
    ...
    $('.btn-upgrade').on('click',function(e){
        ...
        BootstrapDialog.show({
            ...
            message: $('<div></div>').load('account/mastercard/kyc_upgrade_account/'),
            title: 'KYC document upload',
            cssClass: "upgradeCardModal0001"
        });
    });
    ...
    function loadCurrencyNotice() {
        ...
        var more3 = gLangACC_PLA_EXTRA_FEE;
        ...
        if(more3 && $('#card_type').val() == 'plastic'){
            var countryCode = $('#country2').val();
            var countryName = $('#country2 option:selected').text();
            var extraFee = parseFloat(setting_ext[countryCode][sl]) + cur;
            var total = parseFloat(setting_ext[countryCode][sl]) + parseFloat(setting[sl]);
            total = total.formatMoney(2, '.', ',');
            more3 = more3.replace('%2', '<span style="background: #333;color:#fff;">' + extraFee + '</span>');
            more3 = more3.replace('%1', countryName);
            more3 = more3.replace('%3', '<span style="background: #333;color:#fff;">'+ total + cur + '</span>');
            if( parseFloat(setting_ext[countryCode][sl]) > 0){
                text += more3;
            }
        }
    ...
    $(document).ready(function () {
        $('#id_expiry_date, #id_issue_date').datepicker({
            format: 'dd-mm-yyyy'
        });
        $('#dob').datepicker({
            format: 'dd-mm-yyyy',
