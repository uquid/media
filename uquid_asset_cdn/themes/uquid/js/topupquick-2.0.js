(function ($) {
    ...
    window.showAllChoicesForUkUSDestinationOperators = function(data){
        ...
        $(document).trigger('uqtopupTopupOperatorLoaded');
    }
    ...
    window.showAvailableProductsForDestination = function (data) {
        ...
        var gbp = convertCurrencyToCode($('#coin_payee').val());
        ...
        $(document).trigger('uqtopupTopupProductLoaded');
    }
    ...
    window.showAvailableServicesForDestination = function (data) {
        ...
        $(document).trigger('uqtopupServicesLoaded');
    }
    ...
    window.showAvailableOperators = function (data, country_id, service_id) {
        ...
        $('#operatorAvailList').html(html);
        $(document).trigger('uqtopupOperatorsLoaded');
    }
    ...
    window.showAvailableProductsOfOperatorServices = function (data, country_id, service_id, operator_id) {
        ...
        $('#productsAvailList').html(html);
        $(document).trigger('uqtopupProductsLoaded');
    }
    ...
    window.showAvailableProductsOfOperatorServices_PLNAccountIdSpecial = function (serverDataRes, localDataObj, nextBtnSelector) {
        ...
        html += '<div class="amounts2" style="margin-left:0px;width:100%;" data-pln-account-id="' + localDataObj.plnAccountId + '" data-product-id="' + localDataObj.product_id + '" data-price="' + priceAtm + '" data-service-id="' + localDataObj.service_id + '" data-operator-id="' + localDataObj.operator_id + '" data-country-id="' + localDataObj.country_id + '" data-optionselect="' + localDataObj.data_optionselect + '">';
        html += '<a class="btn" href="javascript:;" onclick="javascript: return selectProductTrigger_Services(this)">';
    ...
    window.resetAllSelectedValue = function () {
        ...
        $('#payment_type_selected').val('');

        // Call clear cookies
        // clearTopupCookie();
    }

    function clearTopupCookie() {
        $.removeCookie('uqtopup_coin',  { path: '/' });
        $.removeCookie('uqtopup_destination_number',  { path: '/' });
        $.removeCookie('uqtopup_operatorId',  { path: '/' });
        $.removeCookie('uqtopup_productId',  { path: '/' });
        $.removeCookie('uqtopup_serviceId',  { path: '/' });
        $.removeCookie('uqtopup_countryId',  { path: '/' });
        $.removeCookie('uqtopup_optionselect',  { path: '/' });
    }

    function initFromCookie(){
        var uqtopup_coin = $.cookie('uqtopup_coin');
        var uqtopup_destination_number = $.cookie('uqtopup_destination_number');
        var uqtopup_operatorId = $.cookie('uqtopup_operatorId');
        var uqtopup_productId = $.cookie('uqtopup_productId');
        var uqtopup_serviceId = $.cookie('uqtopup_serviceId');
        var uqtopup_countryId = $.cookie('uqtopup_countryId');
        var uqtopup_topupproduct = $.cookie('uqtopup_topupproduct');
        var uqtopup_topuptype = $.cookie('uqtopup_topuptype');
        var hasCookies = function () {
            var b1 = uqtopup_coin && uqtopup_countryId && uqtopup_serviceId && uqtopup_productId && uqtopup_destination_number && uqtopup_operatorId;
            var b2 = uqtopup_destination_number && uqtopup_coin && uqtopup_topupproduct && uqtopup_topuptype && uqtopup_topuptype == 'topup';
            return b1 || b2;
        }

        if (hasCookies()){
            $('.umask').removeClass('hidden');
        }

        $(document).on('initCountryOK', function () {
            if (hasCookies()){
                setTimeout(function () {
                    $('#coin_payee').val(uqtopup_coin);
                    if (uqtopup_topuptype == 'topup'){
                        $('.check-number-btn').trigger('click');
                    } else {
                        $('.check-services-btn').trigger('click');
                    }
        ...
        $(document).on('uqtopupServicesLoaded', function () {
            if (hasCookies()){
                setTimeout(function () {
                    $('#servicesAvailList [data-service-id='+uqtopup_serviceId+'] a').trigger('click');
                }, 800)
            }
        });

        $(document).on('uqtopupOperatorsLoaded', function () {
            if (hasCookies()){
                setTimeout(function () {
                    $('#operatorAvailList .operatorWrapper [name=select_operator][data-operator-id='+uqtopup_operatorId+'][data-country-id='+uqtopup_countryId+'][data-service-id='+uqtopup_serviceId+']').trigger('click');
                }, 800)
            }
        });

        $(document).on('uqtopupTopupOperatorLoaded', function () {
            if (hasCookies()){
                setTimeout(function () {
                    $('#operatorAvailList .operatorWrapper [name=select_operator][data-operator-id='+uqtopup_operatorId+'][data-country-id='+uqtopup_countryId+'][data-service-id='+uqtopup_serviceId+']').trigger('click');
                }, 800)
        ...
        $(document).on('uqtopupProductsLoaded', function () {
            if (hasCookies()){
                setTimeout(function () {
                    $('#productsAvailList [data-product-id='+uqtopup_productId+'][data-operator-id='+uqtopup_operatorId+'][data-country-id='+uqtopup_countryId+'][data-service-id='+uqtopup_serviceId+'] a').trigger('click');
                    $('.umask').addClass('hidden');
        ...
        $(document).on('uqtopupTopupProductLoaded', function () {
            if (hasCookies()){
                setTimeout(function () {
                    $('#productsAvailList [data-product='+uqtopup_topupproduct+'] a').trigger('click');
                    $('.umask').addClass('hidden');
    ...
    $(document).ready(function () {
                ...
                UQAJAX.post({
                    ...
                    success: function (resp) {
                        if (resp && resp.status){
                            ...
                            $(document).trigger('initCountryOK')
                        }
        ...
        $(".check-number-btn").on('click', function (e) {
            ...
            if (form.valid()) {
                ...
                countryData.coin = $('#coin_payee').val();
        ...
        $(".send-topup-btn").on('click', function (e) {
            ...
            post.action = 'send';
            post.payment_type_selected = $('#payment_type_selected').val();
            post.hdhashing = $('#hdhashing').val();
            post.coin = $('#coin_payee').val();
                ...
                success: function (res) {
                    ...
                    if (!res.status) {
                        ...
                        if(post.payment_type_selected == ''){
                            $('#payment_type_selected').val(res.payment_type_selected);

                            UQCALLBACK(res.token, function () {
                                var m = BootstrapDialog.show({
                                    size: BootstrapDialog.SIZE_WIDE,
                                    message: $('<div></div>').load(res.modalUrl),
        ...
        $('.topup_history').on('click',function (e) {
            var h = $('#hdhashing').val();
            BootstrapDialog.show({
                size: BootstrapDialog.SIZE_WIDE,
                message: $('<div></div>').load('account/topup/history/?hashing='+h),
                title: "Topup history",
                onshow: function (dialog) {
                    dialog.$modal.find('.modal-lg').addClass('modal-xxl');
                }
            });
        })
    ...
    function initSocket() {
        var socket = io.connect('https://uquidcoin.com:9222', {secure: true});
        socket.emit("joinRoom", {hash: socketRoom});
        socket.on('uqtopup', function (data) {
            var color = 'green';
            if(data.status){
                var color = 'green';
                $(document).find('#topupDepositCountDown_controll').text('Completed.');
            }else{
                var color = 'red';
                $(document).find('#topupDepositCountDown_controll').text('Cancelled.');
            }
            iziToast.show({
                title: 'Updates: ',
