(function ($) {
    ...
    function initIcoDeposit() {
        ...
        $(".direct_deposit_uqblx").on("click", function (e) {
            ...
            BootstrapDialog.show({
                size: BootstrapDialog.SIZE_WIDE,
                message: $('<div>loading...</div>').load('account/deposit/ico_uqblx/'),
                title: "Buy UQC token with " + name
            });
        });
    ...
    function refreshRAR() {
        $('.myRARBlx').addClass('fa-spin');
        UQAJAX.post({
            url: 'account/async/rar_uqc_ico',
            success: function (res) {
                if (res.status) {
                    $('#myRARBlx').text(res.message);
                } else {
                    reload();
                }

                $('.myRARBlx').removeClass('fa-spin');
            },
            error: function () {
                console.warn('Server error');
                $('.myRARBlx').removeClass('fa-spin');
    ...
    function showTransactionDetails(hash) {
        BootstrapDialog.show({
            size: BootstrapDialog.SIZE_WIDE,
            message: $('<div></div>').load('account/async/ico_transaction_reveal/?hash=' + hash),
