//TOPUP
var showAvailableProductsForDestination = function (data) {
    var html = '<div class="row">';
    html += '<div class="col-sm-6 text-center">';
    html += '<img src="https://operator-logo.transferto.com/logo_operator/logo-' + data['operatorid'] + '-1.png" alt="' + data['operator'] + '">';
...
var showAvailableServicesForDestination = function (data) {
...
var showAvailableOperators = function (data, country_id, service_id) {
    var filter = [];
    ...
    if (filter.length > 0) {
        ...
        for (var i = 0; i < filter.length; i++) {
            ...
            html += '<label for="op_' + filter[i].operator_id + '"><img src="https://operator-logo.transferto.com/logo_operator/logo-' + filter[i].operator_id + '-1.png" alt="' + filter[i].operator + '">';
...
var showAvailableProductsOfOperatorServices = function (data, country_id, service_id, operator_id) {
...
var showAvailableProductsOfOperatorServices_PLNAccountIdSpecial = function (serverDataRes, localDataObj, nextBtnSelector) {
...
var selectProductTrigger_Services = function (_this) {
...
var selectOperatorTrigger = function (_this) {
    ...
    UQAJAX.post({
        ...
        data: {
            ...
            action: 'checkAllProductsOfOperator',
        ...
        success: function (res) {
            if (!res.status) {
                ...
                showAvailableProductsOfOperatorServices(res.data, country_id, service_id, operator_id);
            }
        },
        error: function () {
...
//
var selectOperatorTriggerSpecial = function (_this) {
...
var checkPLNaccountID = function (_this) {
    var target = _this;
...
//TOPUP
var selectProductTrigger = function (index) {
    if (blockedSelect) return false;

    $('#productsAvailList').find('.amounts2').removeClass('selected')
    $('#productsAvailList').find('.amounts2').eq(index).addClass('selected');
    $('#continueButton').attr('disabled', false);
    $('#continueButton').attr('data-index', index);

    var productAmt = $('#productsAvailList').find('.amounts2').eq(index).find('a').clone().children().remove().end().text();
    var priceAmt = $('#productsAvailList').find('.amounts2').eq(index).find('span').text();
...
var selectServiceTrigger = function (service_id) {
    if (blockedSelect) return false;
...
var resetAllSelectedValue = function () {
            ...
            UQAJAX.post({
                ...
                success: function (res) {
                    if (!res.status) {
                        ...
                        showAvailableProductsForDestination(res.data);
                    }
                },
                error: function () {
                    console.warn('Status returns failure');
    ...
    $(".check-services-btn").on('click', function (e) {
        ...
        if (form.valid()) {
            ...
            UQAJAX.post({
                ...
                data: {
                    destinationPhone: destinationPhone,
                    action: 'checkAllServicesForDestination'
                },
                ...
                success: function (res) {
                    if (!res.status) {
                        ...
                        showAvailableServicesForDestination(res.data);
                    }
                },
                error: function () {
                    console.warn('Status returns failure');
